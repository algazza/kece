...
<section class="box-border p-0 m-0 bg-gray-200 h-screen font-poppins overflow-x-hidden">
  <div class="grid gap-8 grid-cols-1 md:grid-cols-2 lg:ml-[5rem] md:ml-[5rem] sm:ml-[5rem] xss:ml-[2rem] mr-[1rem] mt-20 mb-10">
    ...
    <article class="relative flex flex-col justify-end overflow-hidden rounded-2xl w-full mx-auto mt-1 cursor-pointer group hover:bg-gray-200 duration-300" style="padding-top: 37.5%;">
      <img src="{{ asset('image/public/banner/' . $item->image) }}" alt="Banner Image" class="absolute inset-0 h-full w-full object-cover">
      <div class="z-10 gap-y-1 bg-white border shadow-md rounded-md text-gray-500 overflow-hidden leading-6 absolute bottom-4 right-4 font-poppins text-sm px-3 hidden xs:block">
      ...
      <div class="absolute z-10">
        <form id="image-update-form-{{ $item->id }}" action="{{ route('banner.update', $item->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
...
<script>
  ...
  // Update banner image with dynamic form id
  @foreach($banner as $item)
    const updateImageInput{{ $item->id }} = document.getElementById('update-image-{{ $item->id }}');
    updateImageInput{{ $item->id }}.addEventListener('change', function() {
      const form = document.getElementById('image-update-form-{{ $item->id }}');
      form.submit();
    });
  @endforeach
