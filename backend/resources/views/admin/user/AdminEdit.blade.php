    ...
    <div class="lg:w-[80%] md:w-[90%] xs:w-[96%] mx-auto flex gap-4">
        <div
            ...
            <div class="">
                ...
                <form enctype="multipart/form-data" method="post" action="{{ route('admin.update', $admin->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="w-full rounded-sm bg-[url('https://images.unsplash.com/photo-1449844908441-8829872d2607?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w0NzEyNjZ8MHwxfHNlYXJjaHw2fHxob21lfGVufDB8MHx8fDE3MTA0MDE1NDZ8MA&ixlib=rb-4.0.3&q=80&w=1080')] bg-cover bg-center bg-no-repeat items-center">
                        <!-- Profile Image -->
                        <div class="mx-auto flex justify-center w-[141px] h-[141px] bg-blue-300/20 rounded-full bg-cover bg-center bg-no-repeat" style="background-image: url('{{ asset('image/admin/' . $admin->image) }}');">
                    ...
                    <div class="flex lg:flex-row md:flex-col sm:flex-col xs:flex-col gap-2 justify-center w-full">
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" class="text-grey p-4 w-full border-2 rounded-lg dark:text-gray-200 dark:border-gray-600 dark:bg-gray-800" name="no_handphone" value="{{ old('no_handphone', $admin->no_handphone) }}">
                        </div>
                    </div>
