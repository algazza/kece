            ...
            <section class="box-border p-0 m-0 bg-gray-200 text-center justify-center items-center overflow-hidden font-poppins ">
                    ...
                    @foreach ($penghargaan as $item)
                        <a class="p-8 max-w-lg bg-white rounded-2xl shadow-lg duration-300 flex flex-col items-center"href="#">
                            ...
                            <div class="mt-5 flex gap-2">
                                ...
                                <button type="button" class="inline-flex items-center rounded-md bg-white border-gray-500 border px-3 py-2 text-sm font-medium leading-4 text-black shadow-sm duration-300 hover:bg-red-600">Edit</button>
                            </div>
                            </div>
                        </a>
                    @endforeach
                ...
                <div id="modelConfirm" class="fixed z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 ">
                    <div class="relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-md">
                        <form action="{{ route('penghargaan.update', $penghargaanFind->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="flex flex-col items-center justify-center text-center cursor-pointer gap-1 p-[1rem]">
                                ...
                                <div class="mt-4 px-[2rem] w-full">
                                    <textarea type="text" name="nama_penghargaan" class="rounded-md block p-2.5 w-full z-20 text-sm text-gray-900 shadow-md text-center shadow-gray-200" placeholder="Keterangan">{{ $penghargaanFind->nama_penghargaan }}</textarea>
                                </div>
                                <div class="mt-4 mb-2">
                                    <a href="{{ route('penghargaan.index') }}" class="inline-flex items-center rounded-md border border-transparent bg-red-600 px-10 py-2 text-sm font-medium leading-4 text-white shadow-sm hover:bg-red-700 hover:scale-95 duration-300"
                                            >Close</a>
                                   <button type="submit" class="inline-flex items-center rounded-md border-[1px] bg-white border-gray-300 hover:border-gray-400 px-10 py-2 text-sm leading-4 text-black font-semibold hover:text-white shadow-sm hover:bg-gray-300 hover:scale-95 duration-300">Update</button>
                               </div>
                            </div>            
                        </form>
                    </div>
